            ...
            <div className="relative">
              ...
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2">
                <div className="flex items-center space-x-1">
                  <kbd className="px-2 py-1 bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 text-xs rounded-md font-medium border border-gray-200 dark:border-slate-600">⌘</kbd>
                  <kbd className="px-2 py-1 bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 text-xs rounded-md font-medium border border-gray-200 dark:border-slate-600">K</kbd>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              <Menu.Items className="absolute right-0 top-full mt-2 w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-200/60 dark:border-slate-600/60 overflow-hidden z-50 focus:outline-none backdrop-blur-sm">
                ...
                <div className="px-6 py-4 bg-gradient-to-r from-slate-50 via-blue-50/50 to-indigo-50/50 dark:from-slate-700 dark:via-blue-900/30 dark:to-indigo-900/30 border-b border-gray-100/60 dark:border-slate-600/60">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-300">{unreadCount} unread messages</p>
                    </div>
                ...
                <div className="max-h-96 overflow-y-auto scrollbar-modern">
                  {notifications.map((notification, index) => (
                    <Menu.Item key={notification.id}>
                      {({ active }) => (
                        <div className={`px-6 py-4 transition-colors duration-200 cursor-pointer ${
                          active ? 'bg-gradient-to-r from-blue-50/50 to-indigo-50/30 dark:from-blue-900/20 dark:to-indigo-900/20' : ''
                        } ${
                          notification.unread ? 'bg-blue-50/20 dark:bg-blue-900/20 border-l-3 border-blue-500 dark:border-blue-400' : ''
                        } ${
                          index !== notifications.length - 1 ? 'border-b border-gray-50 dark:border-slate-700' : ''
                        }`}>
                          <div className="flex items-start space-x-4">
                            <div className={`w-3 h-3 rounded-full mt-1.5 flex-shrink-0 ${
                              notification.type === 'lead' ? 'bg-gradient-to-r from-blue-500 to-blue-600' :
                              notification.type === 'billing' ? 'bg-gradient-to-r from-emerald-500 to-green-500' :
                              'bg-gradient-to-r from-gray-400 to-gray-500'
                            } shadow-sm`}></div>
                            
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-1">
                                <h4 className="font-semibold text-gray-900 dark:text-gray-100 text-sm truncate pr-2">{notification.title}</h4>
                                {notification.unread && (
                                  <span className="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0"></span>
                                )}
                              </div>
                              <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed mb-2">{notification.message}</p>
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">{notification.time}</span>
                                <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                                  notification.type === 'lead' ? 'bg-blue-100 text-blue-700' :
                                  notification.type === 'billing' ? 'bg-emerald-100 text-emerald-700' :
                                  'bg-gray-100 text-gray-700'
                                }`}>
                                  {notification.type === 'lead' ? 'Lead' :
                                   notification.type === 'billing' ? 'Billing' : 'System'}
                                </span>
                              </div>
                            </div>
                          </div>
                ...
                <div className="px-6 py-3 bg-gray-50/80 dark:bg-slate-700/50 border-t border-gray-100 dark:border-slate-600">
                  <Menu.Item>
                    {({ active }) => (
                      <button className={`w-full text-center py-2 px-4 rounded-xl font-medium text-sm transition-all duration-200 ${
                        active ? 'bg-blue-600 dark:bg-blue-500 text-white shadow-md transform scale-[0.98]' : 'text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/30'
                      }`}>
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      <button className={`w-full flex items-center space-x-3 px-6 py-3 text-left transition-all duration-200 ${
                        active ? 'bg-gradient-to-r from-blue-50 to-indigo-50/50 dark:from-blue-900/20 dark:to-indigo-900/20 border-r-2 border-blue-500 dark:border-blue-400' : ''
                      }`}>
                        <div className={`p-2 rounded-xl transition-colors duration-200 ${
                          active ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300'
                        }`}>
                          <FiUser className="w-4 h-4" />
                        </div>
                        <div>
                          <span className="text-sm font-medium text-gray-900 dark:text-gray-100">My Profile</span>
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      <button className={`w-full flex items-center space-x-3 px-6 py-3 text-left transition-all duration-200 ${
                        ...
                        <div>
                          <span className="text-sm font-medium text-gray-900 dark:text-gray-100">Platform Settings</span>
                          <p className="text-xs text-gray-500 dark:text-gray-400">System configuration</p>
                        </div>
                      </button>
                    )}
                  </Menu.Item>
                    ...
                    {({ active }) => (
                      <button className={`w-full flex items-center space-x-3 px-6 py-3 text-left transition-all duration-200 ${
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <div>
                              <span className="text-sm font-medium text-gray-900 dark:text-gray-100">Billing & Revenue</span>
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      <button className={`w-full flex items-center space-x-3 px-6 py-3 text-left transition-all duration-200 ${
                        ...
                        <div>
                          <span className="text-sm font-medium text-gray-900 dark:text-gray-100">API & Integrations</span>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Developer access</p>
                        </div>
                      </button>
                    )}
                  </Menu.Item>
                      ...
                      <button 
                        ...
                        <div>
                          <span className={`text-sm font-medium ${
                            active ? 'text-red-700 dark:text-red-300' : 'text-red-600 dark:text-red-400'
                          }`}>Sign Out</span>
            ...
            >
              <Menu.Items className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-gray-200/60 dark:border-slate-600/60 overflow-hidden z-50 focus:outline-none">
                <div className="py-2">
                  <Menu.Item>
                    {({ active }) => (
                      <button className={`w-full flex items-center space-x-3 px-4 py-2.5 text-left transition-colors duration-200 ${
                        active ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'text-gray-700 dark:text-gray-200'
                      }`}>
                        <FiSettings className="w-4 h-4" />
                    ...
                    {({ active }) => (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <span className="w-4 h-4 text-center text-lg">{isDark ? '☀️' : '🌙'}</span>
